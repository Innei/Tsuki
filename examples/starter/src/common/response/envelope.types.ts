...
export const withMeta = <T>(data: T, meta: ResponseMeta): ExplicitSuccessEnvelope<T> =>
  Object.defineProperty({ data, meta }, SUCCESS_ENVELOPE_MARK, {
    enumerable: false,
    value: true,
  }) as ExplicitSuccessEnvelope<T>;

export const isExplicitSuccessEnvelope = <T>(value: unknown): value is ExplicitSuccessEnvelope<T> =>
  typeof value === 'object' &&
  value !== null &&
...
export const isErrorEnvelope = (envelope: ResponseEnvelope): envelope is ErrorEnvelope =>
  'error' in envelope;
