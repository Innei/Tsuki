...
export function createAppException<C extends AppErrorCode>(...args: AppErrorArgs<C>): AppException {
  const [code, payload] = args as [AppErrorCode, unknown];
  const definition = APP_ERROR_DEFINITIONS[code] as RuntimeAppErrorDefinition;
  const message =
    typeof definition.message === 'function' ? definition.message(payload) : definition.message;
