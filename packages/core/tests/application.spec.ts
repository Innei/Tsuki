...
function FactoryParam() {
  return (target: object, propertyKey: string | symbol, parameterIndex: number) => {
    const existing = (Reflect.getMetadata(ROUTE_ARGS_METADATA, target, propertyKey) ||
...
describe('HonoHttpApplication internals', () => {
  ...
  it('extracts middleware metadata and lifecycle targets across variants', async () => {
    ...
    expect(internals.extractMiddlewareMetadata(InlineMiddleware)).toEqual({
      path: '/decorated-inline',
      priority: 7,
    });
    ...
    const fallbackMerged = internals.resolveMiddlewareDefinition(
      { handler: instance } as MiddlewareDefinition,
      {
        path: '/fallback',
        priority: 3,
      },
    ...
    expect(() => internals.getEnhancerType(Symbol('unknown-token'))).toThrowError(
      /Unknown enhancer token/,
    );
  ...
  it('registers global middlewares across array and string paths', async () => {
    ...
    internals.useGlobalMiddlewares(
      arrayDefinition,
      decoratedDefinition,
      regexDefinition,
      fallbackDefinition,
    ...
    try {
      ...
      const verboseMessage = verboseSpy.mock.calls.find(
        (call): call is [string, string] =>
          typeof call[0] === 'string' && call[0].includes('AnonymousMiddleware'),
      );
  ...
  it('sorts global middlewares using default priority when values are undefined', async () => {
    ...
    try {
      ...
      expect(appAny.globalEnhancers.middlewares).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ path: '/low' }),
          expect.objectContaining({ path: '/high' }),
        ]),
      );
  ...
  it('normalizes empty paths to root', async () => {
    ...
    const buildPath = (
      app as unknown as {
        buildPath: (
          controller: { prefix: string; bypassGlobalPrefix: boolean },
          routePath: string,
        ) => string;
      }
    ).buildPath.bind(app);
...
describe('Container resolution policy', () => {
  ...
  it('supports provider aliasing via useExisting and useClass', async () => {
    ...
    @Module({
      providers: [
        { provide: Base as unknown as Constructor, useExisting: Impl as unknown as Constructor },
        Impl,
      ],
      controllers: [],
    })
...
describe('APP_* providers and auto-registration of decorator enhancers', () => {
  ...
  it('applies APP_INTERCEPTOR as a global interceptor (useClass)', async () => {
    ...
    @Module({
      ...
      providers: [
        { provide: APP_INTERCEPTOR as unknown as Constructor, useClass: AppInterceptor2 },
        AppInterceptor2,
      ],
    })
    class IAppModule {}
  ...
  it('auto-registers class decorators for interceptors/filters/pipes without providers', async () => {
    ...
    class InlineInterceptor2 implements Interceptor {
      async intercept(_ctx: ExecutionContext, next: CallHandler): Promise<FrameworkResponse> {
        const result = await next.handle();
        const text = await result.text();
        return new Response(`${text}|inlined`, {
          headers: result.headers,
          status: result.status,
        }) as FrameworkResponse;
      }
