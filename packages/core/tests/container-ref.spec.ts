...
describe('ContainerRef utility', () => {
  ...
  it('includes non-constructor token names in unregistered token errors', () => {
    ...
    expect(() => container.resolve('string-token')).toThrowError(
      /Cannot resolve unregistered token string-token/,
    );
  ...
  it('falls back to AnonymousToken label when resolving undefined tokens', () => {
    ...
    expect(() => container.resolve(undefined as any)).toThrowError(
      /Cannot resolve unregistered token AnonymousToken/,
    );
