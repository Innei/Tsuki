...
function isOnModuleInitHook(value: unknown): value is OnModuleInit {
  return (
    typeof value === 'object' &&
    value !== null &&
    typeof (value as OnModuleInit).onModuleInit === 'function'
  );
}
...
function isOnModuleDestroyHook(value: unknown): value is OnModuleDestroy {
  return (
    typeof value === 'object' &&
    value !== null &&
    typeof (value as OnModuleDestroy).onModuleDestroy === 'function'
  );
}
...
export class HonoHttpApplication {
  ...
  constructor(
    ...
    const rawModuleName = (this.rootModule as { name?: string }).name ?? '';
    this.moduleName =
      rawModuleName && rawModuleName.trim().length > 0 ? rawModuleName : 'AnonymousModule';
   ...
   */
  private getEnhancerType(
    token: InjectionToken,
  ): 'guard' | 'pipe' | 'interceptor' | 'filter' | 'middleware' {
  ...
  private isHttpMiddleware(value: unknown): value is HttpMiddleware {
    return (
      typeof value === 'object' &&
      value !== null &&
      typeof (value as HttpMiddleware).use === 'function'
    );
  }
  ...
  private mergeMiddlewareMetadata(
    primary?: MiddlewareMetadata,
    fallback?: MiddlewareMetadata,
  ): MiddlewareMetadata {
    return {
  ...
  private registerRegularProvider(config: any): void {
    ...
    if ('useValue' in config) {
      ...
      this.enqueueLifecycleResolver(() => config.useValue, undefined, true);
      return;
    }
  ...
  private registerMiddlewareDefinitions(definitions: MiddlewareDefinition[]): void {
    ...
    let normalized = definitions.map((definition) =>
      this.normalizeMiddlewareDefinition(definition),
    );
  ...
  private registerClassProvider(
    ...
    if (isSingleton) {
      this.enqueueLifecycleResolver(() => this.container.resolve(token as any), useClass);
    }
  }
  ...
  private registerController(controller: Constructor): void {
    ...
    for (const route of routes) {
      ...
      this.app.on(method, fullPath, async (context: Context) => {
        const handler = Reflect.get(controllerInstance, route.handlerName) as (
          ...args: any[]
        ) => any;
  ...
  private async executeInterceptors(
    ...
    const callHandler: CallHandler = {
      handle: async (): Promise<FrameworkResponse> =>
        this.ensureResponse(honoContext, await finalHandler()),
    };
  ...
  private async resolveArguments(
    ...
    const maxIndex =
      paramsMetadata.length > 0 ? Math.max(...paramsMetadata.map((item) => item.index)) : -1;
