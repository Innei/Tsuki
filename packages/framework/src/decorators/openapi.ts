...
export function ApiDoc(options: ApiOperationOptions): ClassDecorator & MethodDecorator {
  return (target: any, propertyKey?: string | symbol) => {
    if (propertyKey) {
      const existing = (getStore(target, API_OPERATION_METADATA, propertyKey) ||
...
export function getApiTags(target: object, propertyKey?: string | symbol): string[] {
  const store = propertyKey
    ...
    : (getStore((target as Constructor).prototype ?? target, API_TAGS_METADATA) as
        | string[]
        | undefined);
  return store ? [...store] : [];
}
...
export function getApiDoc(target: object, propertyKey?: string | symbol): ApiOperationOptions {
  const store = propertyKey
    ...
    : (getStore((target as Constructor).prototype ?? target, API_OPERATION_METADATA) as
        | ApiOperationOptions
        | undefined);
  return store ? { ...store } : {};
}
