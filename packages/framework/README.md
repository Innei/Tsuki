...
- `tests/`: Covers DI, decorator metadata, route execution, and exception handling scenarios — recommended starting point for understanding the framework.

---

For more details, consult `CLAUDE.md` in the repository root (detailed development guide for AI agents) or read the `src/` directory source code directly. Feel free to extend modules, enhancers, and tooling based on business needs.
